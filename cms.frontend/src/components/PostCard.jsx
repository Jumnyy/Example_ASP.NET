...
const PostCard = ({ post }) => {
    ...
    // Biến xử lý đường dẫn ảnh an toàn (Đã fix lỗi thiếu dấu gạch chéo '/')
    const getImageUrl = () => {
        const imgPath = post.imageUrl || post.ImageUrl;
        if (!imgPath) return "https://placehold.co/400x400/eadeca/7a685c?text=Solisz+Story";

        // Nếu trong DB đã có sẵn đường dẫn tuyệt đối (http/https) thì giữ nguyên
        if (imgPath.startsWith('http')) return imgPath;

        // Đảm bảo luôn có dấu gạch chéo '/' hợp lệ trước tên file
        ...
        <div className={styles.vintagePostCard}>
            ...
            <div className={styles.imageWarmWrapper}>
                <Link to={`/blog/${post.id || post.Id || '#'}`}>
                    <img
                        src={getImageUrl()} // Đã đưa hàm xử lý ảnh an toàn vào đây
            ...
            <div className={styles.chillPostBody}>
                ...
                <p className={styles.postMeta}>
                    <i className="fa-regular fa-bookmark"></i> nhật ký 2hand • bởi Solisz
                </p>
